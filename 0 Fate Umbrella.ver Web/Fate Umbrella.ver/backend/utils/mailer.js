...
export async function sendVerificationEmail(toEmail, token, username) {
    const frontendUrl = process.env.DYNAMIC_FRONTEND_URL || process.env.FRONTEND_URL;
    const verifyLink = `${frontendUrl}/verify/${token}`;

    const mailOptions = {
        from: '"CunnyWorks" ' + process.env.EMAIL_USER ,
        to: toEmail,
        subject: 'Verificacion de Cuenta | Fate/Umbrella.ver',
        html: `
            <p>Hola ${username || ''},</p>
            <p>Gracias por registrarte en <b>Fate/Umbrella.ver</>. Para activar tu cuenta, hacé click en el siguiente enlace:</p>
